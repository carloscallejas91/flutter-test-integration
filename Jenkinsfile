    ...
    stages {
            ...
            steps {
                script {
                    ...
                    withCredentials([file(credentialsId: "${GCP_CREDENTIALS_ID}", variable: 'GCP_KEY_FILE')]) {
                        // CORREÇÃO FINAL: Executa um comando 'docker run' manual via 'bat'.
                        // Isso nos dá controle total sobre os volumes e o diretório de trabalho,
                        // contornando o problema de mapeamento de caminhos do plugin Jenkins no Windows.
                        bat """
                            docker run --rm --pull=never -v "${env.WORKSPACE}:/app" -v "${GCP_KEY_FILE}:/key.json" -w /app ${DOCKER_IMAGE_NAME} sh -c '
                                set -ex
        ...
        stage('Deploy to QA') {
            steps {
                script {
                    bat """
                        docker run --rm --pull=never -v "${env.WORKSPACE}:/app" -w /app ${DOCKER_IMAGE_NAME} sh -c '
                            set -ex
