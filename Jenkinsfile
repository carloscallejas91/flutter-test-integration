...
pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    ...
                    withCredentials([file(credentialsId: "${GCP_CREDENTIALS_ID}", variable: 'GCP_KEY_FILE')]) {
                        // CORREÇÃO: Converte o script para uma única linha e usa '&&' para encadear os comandos.
                        // Usa a sintaxe %VAR% para variáveis de ambiente, que é a correta para 'bat' no Windows.
        ...
        stage('Deploy to QA') {
            steps {
                script {
                    // CORREÇÃO: Converte o script para uma única linha e usa '&&'.
